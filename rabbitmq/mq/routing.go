...
// NewRabbitMqRouting exchange交换机名称 routingKey路由
func NewRabbitMqRouting(exchange string, routingKey string) *RabbitMq {
	// queueName为""
	return NewRabbitMq("", exchange, routingKey, mqURL)
}

// PublishRouting 路由模式生产
...
// ReceiveRouting 路由模式消费
func (mq *RabbitMq) ReceiveRouting() {
	//创建交换机
	err := mq.channel.ExchangeDeclare(
		mq.Exchange,
		// direct
